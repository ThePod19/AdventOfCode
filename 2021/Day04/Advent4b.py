...
for number in calledNumbers:
    for card in bingoCards:
        if bingoCards.index(card) not in XCards:
            for row in card:
                for column in row:
            ...
            if (columnBingo == "true" or rowBingo == "true") and len(XCards) < len(bingoCards)-1:
                XCards.append(bingoCards.index(card))
            elif (columnBingo == "true" or rowBingo == "true") and len(XCards) == len(bingoCards)-1:
